...
require_once "database.php"; // Make sure database.php is properly configured
...
if (isset($_POST["submit"])) {
  ...
  if (empty($firstName) || empty($middleName) || empty($lastName) || empty($age) || empty($contact) || empty($email) || empty($dateOfBirth) || empty($occupation) || empty($ownedPet) || empty($identificationNo) || empty($salary) || empty($sex)) {
    array_push($errors, "All fields are required");
  }

  if (empty($errors)) {
    $sql = "INSERT INTO information (firstName, middleName, lastName, age, contact, email, dateOfBirth, occupation, ownedPet, identificationNo, salary, sex) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    
    $stmt = mysqli_stmt_init($conn);

    if (mysqli_stmt_prepare($stmt, $sql)) {
      mysqli_stmt_bind_param($stmt, 'ssssssssssss', $firstName, $middleName, $lastName, $age, $contact, $email, $dateOfBirth, $occupation, $ownedPet, $identificationNo, $salary, $sex);
                      ...
                      <label class="col-lg-2 control-label">Owned a pet before?</label>
                          <div class="col-lg-2">
                              <div class="form-check form-check-inline">
                                  <input class="form-check-input" type="radio" name="owned" id="ownedYes" value="option1">
                                  <label class="form-check-label" for="ownedYes">Yes</label>
                              </div>
                              <div class="form-check form-check-inline">
                                  <input class="form-check-input" type="radio" name="owned" id="ownedNo" value="option2">
                                  <label class="form-check-label" for="ownedNo">No</label>
                              </div>
                          </div>
