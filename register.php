...
if (isset($_POST["submit"])) {
  ...
  $errors = array();
  if (empty($firstName) || empty($middleName) || empty($lastName) || empty($userType) || empty($email) || empty($password) || empty($confirmPassword)) {
  ...
  $sql = "SELECT * FROM creates WHERE Email = ?";
  ...
  if (count($errors) > 0) {
    ...
    $sql = "INSERT INTO creates (firstName, middleName, lastName, userType, Email, Password) VALUES (?, ?, ?, ?, ?, ?)";
    $stmt = mysqli_stmt_init($conn);
    if (mysqli_stmt_prepare($stmt, $sql)) {
